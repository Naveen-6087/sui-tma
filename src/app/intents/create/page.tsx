...
export default function CreateIntentPage() {
  ...
  const [triggerType, setTriggerType] = useState<"price_below" | "price_above">(
    "price_below",
  );
  ...
  const [encryptionResult, setEncryptionResult] =
    useState<EncryptedIntentResult | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!session?.zkProof) {
        throw new Error(
          "Session not ready for signing. Please wait for ZK proof.",
        );
      }
    ...
    return (
      <Page>
        ...
        >
          ...
          >
            <svg
              style={{ width: 40, height: 40, color: "var(--tma-success)" }}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            >
              ...
              <span
                style={{
                  ...
                  color:
                    side === "buy" ? "var(--tma-success)" : "var(--tma-error)",
                }}
              >
          ...
          {encryptionResult && (
            <div
              className="tma-card animate-fadeIn"
              style={{
                width: "100%",
                maxWidth: 320,
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontFamily: "monospace" }}>
                    {encryptionResult.verification.encryptionMethod}
                  </span>
                </div>
                <div
                  style={{ display: "flex", justifyContent: "space-between" }}
                >
                  ...
                  <span style={{ fontFamily: "monospace" }}>
                    {encryptionResult.verification.threshold}/
                    {encryptionResult.verification.keyServerCount}
                  </span>
                </div>
                <div
                  style={{ display: "flex", justifyContent: "space-between" }}
                >
                  ...
                  <span style={{ fontFamily: "monospace" }}>
                    {encryptionResult.verification.encryptedSize} bytes
                  </span>
                </div>
                {encryptionResult.verification.isRealEncryption && (
                  <div
                  ...
                  >
                    ...
                    <span style={{ fontFamily: "monospace", fontSize: 10 }}>
                      {encryptionResult.verification.packageId.slice(0, 8)}...
                      {encryptionResult.verification.packageId.slice(-6)}
                    </span>
                  </div>
                )}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: "var(--tma-hint-color)" }}>
                  Your intent will be encrypted using Seal. Only the secure
                  enclave can decrypt and execute it.
                </p>
              </div>
            </div>
          </div>

          {error && <div className="tma-error-msg animate-fadeIn">{error}</div>}
  ...
  return (
    <Page>
      ...
      >
        ...
        {/* Trigger Condition */}
        <div
          className="tma-form-group animate-fadeIn"
