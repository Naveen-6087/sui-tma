...
const STATUS_CONFIG: Record<number, { label: string; color: string; bg: string }> = {
  0: { label: 'Active', color: 'var(--tma-success)', bg: 'var(--tma-success-bg)' },
  1: { label: 'Executing', color: 'var(--tma-accent-color)', bg: 'var(--tma-accent-bg)' },
  2: { label: 'Executed', color: 'var(--tma-success)', bg: 'var(--tma-success-bg)' },
  3: { label: 'Cancelled', color: 'var(--tma-hint-color)', bg: 'var(--tma-secondary-bg)' },
  4: { label: 'Expired', color: 'var(--tma-warning)', bg: 'var(--tma-warning-bg)' },
...
export default function IntentsPage() {
  ...
  const loadIntents = async () => {
    ...
    try {
      ...
      const onChainIntents = await fetchUserIntents(session.zkLoginAddress, suiClient);
  ...
  const handleCancel = async (intentId: string) => {
    ...
    try {
      ...
      const tx = buildCancelIntentTx(intentId, PACKAGE_IDS.intentRegistryObject);
  ...
  const activeCount = intents.filter(i => i.status === 0 || i.status === 1).length;
      ...
      <div className="tma-page" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button onClick={() => router.push('/dashboard')} className="tma-back-btn">
        ...
        <div className="tma-stat-row animate-fadeIn">
          ...
          <div className="tma-stat-item">
            <span className="tma-stat-value" style={{ color: 'var(--tma-success)' }}>{activeCount}</span>
        ...
        {/* Filter tabs */}
        <div className="tma-tabs animate-fadeIn" style={{ animationDelay: '0.05s' }}>
          {(['all', 'active', 'completed'] as const).map((f) => (
            <button
              key={f}
              onClick={() => { setFilter(f); hapticFeedback.selectionChanged.ifAvailable(); }}
          ...
          {filteredIntents.length === 0 ? (
            <div className="tma-empty-state animate-fadeIn">
              ...
              <p style={{ color: 'var(--tma-hint-color)', fontSize: 14 }}>
                {filter === 'all' ? 'Create your first trading intent' : `No ${filter} intents`}
              </p>
              {filter === 'all' && (
                <Link href="/intents/create" className="tma-btn" style={{ marginTop: 16 }}>
                  Create Intent
                </Link>
              )}
            </div>
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        <span style={{ fontWeight: 600, fontSize: 16 }}>{intent.pair.replace('_', '/')}</span>
                        <span 
                          className="tma-badge"
                          style={{ 
                      ...
                      <p style={{ fontSize: 13, color: 'var(--tma-hint-color)' }}>
                        {formatTriggerCondition(intent.triggerType, intent.triggerValue, intent.pair)}
                      </p>
                    </div>
                    ...
                    {intent.side && (
                      <span 
                        style={{ 
                          fontWeight: 600, 
                          fontSize: 15,
                          color: intent.side === 'buy' ? 'var(--tma-success)' : 'var(--tma-error)'
                        }}
                      >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', gap: 16, fontSize: 13 }}>
                      {intent.quantity && (
                        <span>
                          <span style={{ color: 'var(--tma-hint-color)' }}>Qty: </span>
                          <span style={{ fontWeight: 500 }}>{intent.quantity}</span>
                        </span>
                      )}
                      <span>
                        <span style={{ color: 'var(--tma-hint-color)' }}>Expires: </span>
