...
export function createTradingBot(token: string): Bot<BotContext> {
  ...
  bot.callbackQuery("cmd_flasharb", async (ctx) => {
    await ctx.answerCallbackQuery();
    await ctx.editMessageText("🔍 *Scanning for Arbitrage...*", {
      parse_mode: "Markdown",
    });
  ...
  bot.callbackQuery(/^margin_confirm_(\w+)_(\d+)_(\d+)$/, async (ctx) => {
    ...
    await ctx.editMessageText("⚡ *Opening Position...*", {
      parse_mode: "Markdown",
    });
  ...
  bot.callbackQuery(/^flasharb_execute_(\d+)$/, async (ctx) => {
    ...
    await new Promise((resolve) => setTimeout(resolve, 1000));
    await ctx.editMessageText(`⚡ *Step 1/4:* Taking flash loan...`, {
      parse_mode: "Markdown",
    });
    ...
    await new Promise((resolve) => setTimeout(resolve, 800));
    await ctx.editMessageText(`⚡ *Step 4/4:* Repaying loan + profit...`, {
      parse_mode: "Markdown",
    });
  ...
  bot.callbackQuery("confirm_trade", async (ctx) => {
    ...
    if (!ctx.session.walletAddress && !DEMO_MODE) {
      await ctx.editMessageText(
        "⚠️ Please connect your wallet first with /connect",
      );
      return;
    }
